...
Top-level channel messages publish into the **channel thread**; thread replies publish into the **per-thread leaf fork** (lazy-created at first reply). When the agent replies to a thread message, `read_thread()` returns the leaf's most recent events (tail). To rewind into older context — the parent message and prior channel chatter — the agent passes a lower `from_seq`; once it drops below the leaf's `seq_offset`, Spool's chain walker transparently surfaces ancestor-thread events. Each transcript entry exposes its `seq`, so the agent has anchors to pass back. The SDK's `[fabric: event seq=N]` prompt preamble surfaces the agent's absolute lineage position so the agent knows when there's prior context worth fetching.
