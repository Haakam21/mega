...
- **Sessions** (the cross-channel mode, `core/spool-loop.ts::startSessions`, gated on `MEGA_SESSIONS_PARENT`): one Spool thread per logical conversation regardless of which provider events arrive over. Fabric's `session_routes` table maps `(tenant, connector_type, deriveForkKey-output)` → session thread; inbound events route by lookup, agent-spawned conversations get sub-forks under the session thread. Depth = 2: sessions at depth 1, per-conversation sub-forks at depth 2; both levels host agents. The SDK wires BOTH `slack-action` and `agentmail-action` MCPs into every turn — per-event provider headers populate the originating channel's MCP fully; the other gets framework headers only, exposing its "start-a-new-conversation" surface (e.g. `agentmail-action.send_message`, or `slack-action.post_message` with explicit `channel`). When the agent in a sub-fork needs the originating context (e.g. "ping me in the Slack thread we started this from"), it calls `read_thread({from_seq: lowest_seq - 100})` to walk back through ancestor events via Spool's chain walker — no need to know the parent thread's name. Operator setup: `scripts/setup-sessions.sh` (creates `mega/sessions` Spool thread + 4 session-routed bindings on fabric + appends `MEGA_SESSIONS_PARENT` to `.env`).
    - **`join_session`** — when the agent recognizes that two separate sessions (different connectors, same logical conversation) belong together, `join_session({into: <other_session_thread>})` fuses them at the Spool level: source's stream freezes at a `thread.joined` marker, future events on the current session redirect to the target, and readers/cursors auto-follow with seqs continuous past the boundary. Spool primitive in spool#4, fabric MCP wrapper in fabric#17.
