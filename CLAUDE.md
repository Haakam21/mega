...
8. Claude composes UX via `slack-action.react({ name })`, `slack-action.post_message({ text })`, and `slack-action.unreact({ name })` — typically reacting `thinking_face` first, posting the reply, then clearing the reaction. Each tool call hops fabric MCP → Spool publish → action-supervisor tail → Slack API. The auto-injected `slack-action.read_thread` tool fetches the fork's transcript (tail by default; lower `from_seq` to walk older — crosses into channel-level history once below the leaf's `seq_offset`).
