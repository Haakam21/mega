...
- **Sessions** (the cross-channel mode, `core/spool-loop.ts::startSessions`, gated on `MEGA_SESSIONS_PARENT`): one Spool thread per logical conversation regardless of which provider events arrive over. Fabric's `session_routes` table maps `(tenant, connector_type, deriveForkKey-output)` → session thread; inbound events route by lookup, agent-spawned conversations get sub-forks under the session thread. Depth = 2: sessions at depth 1, per-conversation sub-forks at depth 2; both levels host agents. The SDK wires BOTH `slack-action` and `agentmail-action` MCPs into every turn — per-event provider headers populate the originating channel's MCP fully; the other gets framework headers only, exposing its "start-a-new-conversation" surface (e.g. `agentmail-action.send_message`, or `slack-action.post_message` with explicit `channel`). When the agent in a sub-fork needs the originating context (e.g. "ping me in the Slack thread we started this from"), it calls `read_thread({from_seq: lowest_seq - 100})` to walk back through ancestor events via Spool's chain walker — no need to know the parent thread's name. Operator setup: `scripts/setup-sessions.sh` (creates `mega/sessions` Spool thread + 4 session-routed bindings on fabric + appends `MEGA_SESSIONS_PARENT` to `.env`).
    - **`join_thread`** — when the agent recognizes that two separate sessions (different connectors, same logical conversation) belong together, `join_thread({into: <other_thread>})` fuses them at the Spool level: source's stream freezes at a `thread.joined` marker, future events on the current thread redirect to the target, and readers/cursors auto-follow with seqs continuous past the boundary. Spool primitive in spool#4, fabric MCP wrapper in fabric#17 (renamed from `join_session` to `join_thread` in fabric#23). **Authz (spool-side):** Owner on both threads (source is *dissolved*, target *absorbs* — both Owner-grade); the join stays within a tenant (the threads' fork-tree roots must share an Owner, else `cross_tenant_join`); and source's membership roster is materialized onto target so the redirect doesn't silently grant access (Owner capped to Writer, no downgrade of an existing target role). The delegated fabric path keeps working because fabric owns the session forks it creates, so it satisfies Owner-on-both for the forks it actually joins.
- **AgentMail** and **Slack** (legacy, gated on `MEGA_AGENTMAIL_PARENT` / `MEGA_SLACK_PARENT`): both brokered by fabric, both run through `core/spool-loop.ts::startForkedChannel`. Agentmail runs flat (`depth: 1`) — one fork per email thread. Slack runs nested (`depth: 2`) — bot → channel → per-thread fork, so `read_thread` on a leaf surfaces channel context when the agent passes a `from_seq` low enough to cross below the fork's `seq_offset`. The SDK does recursive discovery at every level, spawning inbound consumers at depth ≥ 1 and nested discovery cursors at depth < maxDepth; `listChildren` at each level handles startup so existing forks aren't stranded by a persisted cursor that's past their `thread.forked` event. SSE tails reconnect with capped jittered backoff after transient drops. Claude's reply publishes back to the fork; fabric's outbound supervisor tails and dispatches via the provider's API (AgentMail reply / `chat.postMessage`). These coexist alongside `startSessions`; the operator can run any combination by setting the relevant env vars.
- **Interrupt-on-arrival** (SDK's `ForkConsumer`). Per fork, two states: `idle` and `flushing`. The cursor's SSE tail is always streaming — `onArrival(ev)` is synchronous and never blocks on Claude. Idle + accepted event → fire `invokeClaude` immediately (no debounce). Flushing + accepted event → `AbortController.abort()` tree-kills the in-flight `claude --print` and queues the new event in `pending`; on the aborted resolution, the consumer re-fires immediately with `[...batch, ...pending]` so the next turn sees the full carried context. The cursor is acked only on non-aborted completion (`max(batch.seq)+1`), so a crash mid-flush replays the un-acked range on restart. Filtered events (bot-authored, off-topic) ack inline in idle, ride along in the batch's seq range during a flush — they don't enter the prompt or trigger an abort. Routing context (`mcpServers`, `sessionIdFor`) resolves against the *latest* accepted event so reactions/reply-to land on the user's most recent message. Watch `harness.log` for `flush fork=… accepted=N ok=… aborted=…` per turn and `aborted fork=… carried=N` on each interrupt.
...
#### Codex steering (`MEGA_CODEX_MODE=steer`)
When `MEGA_AGENT=codex`, `MEGA_CODEX_MODE` picks the codex *engine*: `exec` (default — one-shot per turn, abort+recombine on a mid-turn event) or `steer`. In `steer` mode the SDK runs a single long-lived `codex app-server` daemon and maps each Spool fork to one app-server **thread**; a new event arriving mid-turn is injected via `turn/steer` into the running turn (no abort, no lost work) instead of restarting. `core/spool-loop.ts` constructs one shared `AppServerClient` + `ThreadRegistry` (`steeringSingletons`) and passes a `steering` block (with a *fork-stable* MCP config — `forkStableMcp`, framework headers only; the agent supplies per-message ids via `read_thread`) into each channel via `makeSteering`. The fork→thread map persists to `MEGA_CODEX_THREADS_PATH` (default `<repo>/.codex-threads.json`, gitignored) so threads resume across restarts. Steering is **codex-only** and opt-in; claude and `codex exec` are unaffected. Design + decisions: `fabric/docs/codex-steering-plan.md`.
...
- **Process-count watchdog** (`core/watchdog.ts`) — every `MEGA_WATCHDOG_INTERVAL_MS` (default 30 s) the harness runs `pgrep -cf` against the default pattern `(^|/)(claude --print|codex exec|codex app-server)` (matches claude, codex one-shot, AND the codex steering daemon, by bare name or absolute path — so `MEGA_CODEX_BIN=/abs/path/codex` is still counted) and warns into `harness.log` if the count exceeds `MEGA_WATCHDOG_THRESHOLD` (default 8). In steer mode the `codex app-server` is a single expected long-lived process — well under the threshold; the count still guards against a leak. Belt-and-suspenders: catches leaks if every other layer lets one through. Pattern is overridable via `MEGA_WATCHDOG_PATTERN`. The interval timer is `unref()`'d so it never blocks process exit.
...
| `MEGA_WATCHDOG_PATTERN` | `(^\|/)(claude --print\|codex exec\|codex app-server)` | `pgrep -f` pattern for the watchdog (matches claude, codex exec, and the codex steering daemon; bare-name or absolute-path) |
...
| `MEGA_CODEX_MODE` | `exec` | Codex engine when `MEGA_AGENT=codex`: `exec` (one-shot per turn) or `steer` (long-lived app-server, mid-turn `turn/steer`). |
| `MEGA_CODEX_THREADS_PATH` | `<repo>/.codex-threads.json` | Where steer mode persists the `fork→{threadId,lastConsumedSeq}` map (resume + crash-replay-skip). |
